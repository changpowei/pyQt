...
def crawler_web(url):
    ...
    try:
        ...
        if video_flag == False:
            content_filtered = garbage_filtering(content_splited)
        else:
...
def garbage_filtering(content_splited):
    content_filtered = []
    for paragraph in content_splited:
        ...
        if end_flag == True:
            ...
            if '\u3000' in paragraph:
                ...
                garbage_flag = False
                content_half = strQ2B(paragraph)
                for garbage_item in garbage_content:
...
if __name__ == '__main__':
    ...
    garbage_content = ['圖:', '資料照', '@', '圖/', '圖為', '取自',
                       '新頭殼newtalk', 'https',
                       '(臉書)', '(美聯社)', '(示意圖)', '(推特)',
                       '(左)', '(右)',
                       '作者:'
                       ]
    garbage_end = ['延伸閱讀', '德國之聲版權聲明', '《工商時報》LINE好友', 'LINE官方帳號', '立即加入NOWnews',
    ...
    org_path = './解析txt/' + data + '.txt'
    save_path = './本文/' + data + '_本文.docx'

    all_txt = read_txt(org_path)
    ...
    num = 0
    for line in all_txt:
        if(line[0]!="共" and line[0]!='\n'):
            if(line[:5] == 'https'):
                num += 1
                content_filtered = crawler_web(line)
        ...
        print("\r",end="")
        print("Scratch progress: {}%: ".format(int(num * 100 / questions)),"▋" * (int(num * 100 / questions) // 2),end="")
        sys.stdout.flush()
